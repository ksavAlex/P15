...
def report_results(results):
    ...
    bb = BIG_BLIND
    bb_per_hand = mean_val / bb * 100
    print(json.dumps({
        "mode": "eval",
        "bb_100": bb_per_hand,
        "std_bb": std_val / bb * 100,
        "ci95_halfwidth": 1.96 * std_val / math.sqrt(len(arr)) / bb * 100,
        "hands": len(results)
    }, indent=2))
...
def cmd_evaluate(args):
    ...
    results = []          # raw hero payoffs
    results_aivat = []    # AIVAT-lite corrected payoffs
    ...
    for h in range(hands):
        ...
        while True:
            ...
            legal = legal_actions(s)
            if not legal:
                v = showdown_value(s)  # hero payoff (BB)
                results.append(v)
    ...
    rep = report_results(results)
    if AIVAT_LITE_ENABLED and results_aivat:
        rep2 = report_results(results_aivat, label="AIVAT-lite")
        # объединённый вывод (если report_results печатает сам — просто вернём 0)
        return 0 if rep == 0 or rep2 == 0 else rep2
    return rep
